...
\item \emph{makeOverallTest} -- gdy opcja ta zostanie ustawiona na \texttt{true}, po uruchomieniu aplikacji zostanie przeprowadzony obszerny test, zawierający w~sobie kombinacje rozgrywek komputerów o~wszystkich poziomach zagnieżdżeń $\leq 6$ na wszystkich mocach zbiorów $>0 $ i~$\leq 7$. Czyli zostanie wykonanych $6*6*6$ rozgrywek. Warto wspomnieć, że podczas tego testu opcja \texttt{maxThinkTime} okazała się niezwykle pomocna.
...
Metoda \texttt{compareSubSeqOverlap} działa na tej samej zaszadzie co metoda \mbox{\texttt{compareSubSeq}}. Różni się tutaj wartość zmiennej \texttt{comparesFitInSequence}, jest ona większa o~$1$, z~takiego samego powodu, co zmienna \texttt{maxSeqSize} z~metody \mbox{\texttt{findOverlap}}. Różni się również podciąg zapisywany do zmiennej \mbox{\texttt{right}}, w~pętli z~6 linii. Pierwszy indeks owego podciągu jest równy indeksowi ostatniego elementu, lewego podciągu, po to by stworzyć nasunięcie.
